...
class CheckoutPage extends StatelessWidget {
  ...
  const CheckoutPage({super.key, required this.tableNumber});

  @override
  Widget build(BuildContext context) {
    return MaterialApp(home: MyCheckoutPage(tableNumber: tableNumber));
  }
}
...
class MyCheckoutPage extends StatefulWidget {
  ...
  const MyCheckoutPage({Key? key, required this.tableNumber}) : super(key: key);
  @override
  ...
  void loadData() async {
    try {
      ...
      final response = await http.get(
        Uri.parse('$appBackendUrl/api/order/dinein/${widget.tableNumber}'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $token',
        },
      ...
      Future.delayed(Duration(seconds: 1), () {
        ...
        setState(() {
          _selectedTable = orderCast['table_number'];
          orders = itemsCast;
          _customerNameController.text = orderCast['customer_name'];
          _orderNumberController.text = orderCast['order_number'];
          _totalPaidController.text = double.parse(
            orderCast['total_paid'].toString(),
          ).toStringAsFixed(2);
        });
      });
    } catch (e) {
          ...
          child: Column(
            ...
            children: [
              Container(
                ...
                child: Center(
                  child: Text(
                    "Please fill in the fields below to checkout order.",
